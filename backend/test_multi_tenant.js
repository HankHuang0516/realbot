...
async function runMultiEntityTest() {
    console.log(`?Се Starting Multi-Entity Independence Test on: ${BASE_URL}\n`);
    ...
    if (entities.activeCount < 2) {
        console.log('\n?ая?  Need at least 2 bound entities to test independence.');
    ...
    if (entity0 && entity1) {
        ...
        if (entity0.message !== entity1.message || entity0.state !== entity1.state) {
            ...
            console.log('\n?ая?  Entities have same state (may be coincidence)');
        }
    }
}
