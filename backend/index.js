...
app.post('/api/client/speak', async (req, res) => {
    ...
    const pushPromises = targetIds.map(async (eId) => {
        ...
        if (entity.webhook) {
            ...
            pushMsg += `\n查看任務面板: GET https://eclaw.up.railway.app/api/mission/dashboard?deviceId=${deviceId}&botSecret=${entity.botSecret}&entityId=${eId}`;
...
app.post('/api/entity/speak-to', async (req, res) => {
    ...
    if (toEntity.webhook) {
        ...
        pushMsg += `\n查看任務面板: GET https://eclaw.up.railway.app/api/mission/dashboard?deviceId=${deviceId}&botSecret=${toEntity.botSecret}&entityId=${toId}`;
