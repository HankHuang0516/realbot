...
Server
?”â??€ devices[deviceId]
    ?”â??€ entities[0-3]  // Each device has 4 independent entities
```

### Multi-Device Isolation Example
```
Device A (deviceId: "phone-alice")
?œâ??€ Entity 0 ??Controlled by Bot Alice
?œâ??€ Entity 1 ??Controlled by Bot Alice2

Device B (deviceId: "phone-bob")
?œâ??€ Entity 0 ??Controlled by Bot Bob (No conflict with Alice Entity 0)
?œâ??€ Entity 1 ??Controlled by Bot Bob2
```
...
## ? ï? Important: Bot Authentication
...
### `register_webhook`
**? ï? REQUIRED:** Register webhook URL immediately after binding to enable Push mode.
...
### ? ï? Webhook Handshake & Best Practices (Crucial for UX)
...
### ? ï? IMPORTANT: Webhook Response Ignored
...
4. **? ï? REQUIRED: Bot immediately calls `/api/bot/register` to enable push notifications**
...
**? ï? Important**: Must save `deviceId`, `entityId`, `botSecret`!
...
| GET /api/client/pending | Poll Messages | ??| ? ï? (Peek count if missing) |
...
Device A (deviceId: "phone-alice")
?œâ??€ Entity 0 ??Controlled by Bot Alice
?”â??€ Entity 1 ??Controlled by Bot Alice2

Device B (deviceId: "phone-bob")
?œâ??€ Entity 0 ??Controlled by Bot Bob (No conflict with Alice Entity 0)
?”â??€ Entity 1 ??Controlled by Bot Bob2
